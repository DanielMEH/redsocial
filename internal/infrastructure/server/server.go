...
func setRoutersServer(app *fiber.App, h *types.HandlersStore) {
	for _, handlerModule := range h.Handlers {

		group := app.Group("/" + handlerModule.Prefix)

		for _, itemsRouter := range handlerModule.Routes {
			group.Add(itemsRouter.Method, itemsRouter.Route, itemsRouter.Handler)
		}
	}
}

func NewFiberServer(lc fx.Lifecycle, h *types.HandlersStore, appsettings *config.AppSettings) *fiber.App {
	// 1. Preparar logs y configuracion de salida y guardado
	logDir := "logs"
	...
	// 3. configurar logs de salida en formato json
	...
	lc.Append(fx.Hook{
		OnStart: func(ctx context.Context) error {
			go func() {
				...
				if err := app.Listen(":" + appsettings.Config.Port); err != nil {
